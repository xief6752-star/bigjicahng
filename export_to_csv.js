...
const csvFilePath = path.join('/Users/mac/.gemini/antigravity/scratch', 'outsideweb-wp-import.csv');
...
db.all("SELECT * FROM articles WHERE type = 'review' OR slug = 'post-2026-best'", (err, rows) => {
    ...
    const headers = ['title', 'subtitle', 'date', 'tag', 'slug', 'score', 'markdown_content'];
    ...
    console.log(`=== Export complete! Single CSV file created at: ${csvFilePath} ===`);
    db.close();
});
